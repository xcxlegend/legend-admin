function ubbEditor(_id)
{
    ...
    this.tToolbarSets['mico'] = ['Bold','TextColor'];
    ...
    this.tXHTML2UBB = function(_strers)
    {
        ...
        if (tstrers)
        {
            ...
            var tReplaceAry = [
                ...
                [/<font style>([^<]*?)<\/font>/igm, function($0, $1) {
                    console.log($0, $1);
                    var tString = $0;
                    return tString;
                }, true],
            ...
            tstrers = tstrers.replace(/<p>/igm, '');
            tstrers = tstrers.replace(/<div>/igm, '');
            tstrers = tstrers.replace(/<font>/igm, '');
            tstrers = tstrers.replace(/<font style>/igm, '');
            tstrers = tstrers.replace(/<\/font>/igm, '');
            tstrers = tstrers.replace(/<font style=\"\">/igm, '');
            tstrers = tstrers.replace(/<\/p>/igm, '\n');
            tstrers = tstrers.replace(/<\/div>/igm, '\n');
            tstrers = tstrers.replace(/\[b\]/igm, '<b>');
            tstrers = tstrers.replace(/\[\/b\]/igm, '</b>');
            if (tstrers.lastIndexOf("\n") == tstrers.length - 1 ){
                tstrers = tstrers.substring(0, tstrers.lastIndexOf('\n'));
            }
    ...
    this.tUBB2XHTML = function(_strers)
    {
        ...
        if (tstrers)
        {
            ...
            var tReplaceAry = [
                ...
                [/\<color=\"([^\>]*)\"\>([^\<]*?)\<\/color\>/igm, '<font style="color: $1">$2</font>', true],
                [/\<color=([^\"\>]*)\>/igm, '<font style="color: $1">', true],
                [/\<\/color=([^\>]*)\>/igm, '</font>', true],
                [/\<color=\"([^\>]*)\"\>([^\<]*?)\<\/color\>/igm, '<font style="color: $1">$2</font>', true],
                [/\<font style=\"\">([^\<]*?)\<\/font\>/igm, '$0', true],
                [/\<font style>([^\<]*?)\<\/font\>/igm, '$0', true],
            ...
            console.log("o:", tstrers)
            tstrers = this.tReplace(tstrers, tReplaceAry);
            // tstrers = this.tHTMLEncode(tstrers);
            console.log("en:", tstrers)
            tstrers = tstrers.replace(/\n/igm, '<br />');
        };
        console.log("html:", tstrers)
        return tstrers;
    };
    ...
    this.tInit = function(_name, _baseURL)
    {
        ...
        if (tObj1)
        {
            ...
            tIframe1.style.width = (tObj1Width - 2) + 'px';
            ...
            if (this.tValue)
            {
                ...
                else
                {
                    ...
                    for (ti = 0; ti < tnValueAry.length; ti ++) tnValueContent = tnValueContent + tnValueAry[ti] + '<br />';
                };
